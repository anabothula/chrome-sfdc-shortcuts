...
let orgs1 = [
  {
    hostname: "trailheadorg559-dev-ed",
    groupName: "trailhead",
    color: "green",
    index: "0",
  },
  {
    hostname: "chamberlainu--fullsbox",
    groupName: " CU--FULL ",
    color: "blue",
    index: "1",
  },
  {
    hostname: "chamberlainu--sjdev",
    groupName: " CU--SJDEV ",
    color: "grey",
    index: "2",
  },
  {
    hostname: "rossu--fullsbox",
    groupName: " DMI--FULL ",
    color: "blue",
    index: "3",
  },
  {
    hostname: "rossu--sjdev",
    groupName: " DMI--SJDEV ",
    color: "grey",
    index: "4",
  },
  {
    hostname: "auc--fullsanbox",
    groupName: " AUC--FULL ",
    color: "blue",
    index: "5",
  },
  ...
  {
    hostname: "adtalem.atlassian.net",
    groupName: " JIRA ",
    color: "blue",
    index: "7",
  },
...
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
  if (changeInfo.url && changeInfo.url.indexOf("LookupPage") === -1) {
...
chrome.tabs.onCreated.addListener(function (tab) {  
  ...
  // chrome.storage.local.set({ sfdcOrgGroups: JSON.stringify(orgs) });
});

async function groupTabs(tabId, tabUrl) {
  const orgs = await getFromStorage("sfdcOrgGroups");
  console.log(orgs);
  const currOrg = orgs.find((org) => tabUrl.indexOf(org.hostname) > -1);
  if (!currOrg) return;
...
async function getFromStorage(storagekey) {
